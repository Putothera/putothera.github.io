...
const ANIMATION_CONFIG = { SMOOTH_DURATION: 600, INITIAL_DURATION: 1500, INITIAL_X_OFFSET: 70, INITIAL_Y_OFFSET: 60 } as const;
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const animationHandlers = useMemo(() => {
    ...
    const updateCardTransform = (offsetX: number, offsetY: number, card: HTMLElement, wrap: HTMLElement) => {
        ...
        const properties = {
            ...
            "--rotate-x": `${round(-(centerY / 5))}deg`, "--rotate-y": `${round(centerX / 4)}deg`,
        };
    ...
    const createSmoothAnimation = (duration: number, startX: number, startY: number, card: HTMLElement, wrap: HTMLElement) => {
        ...
        const targetX = wrap.clientWidth / 2, targetY = wrap.clientHeight / 2;
        const animationLoop = (currentTime: number) => {
            const elapsed = currentTime - startTime, progress = clamp(elapsed / duration), eased = easeInOutCubic(progress);
  ...
  useEffect(() => {
    ...
    const card = cardRef.current, wrap = wrapRef.current;
    if (!card || !wrap) return;
    const pme = handlePointerMove as EventListener, pee = handlePointerEnter as EventListener, ple = handlePointerLeave as EventListener;
    card.addEventListener("pointerenter", pee);
    card.addEventListener("pointermove", pme);
    card.addEventListener("pointerleave", ple);
    ...
    return () => {
      card.removeEventListener("pointerenter", pee);
      card.removeEventListener("pointermove", pme);
      card.removeEventListener("pointerleave", ple);
      animationHandlers.cancelAnimation();
    };
      ...
      <section ref={cardRef} className="pc-card">
        <div className="pc-inside">
          ...
          <div className="pc-content pc-avatar-content">
            <img className="avatar" src={avatarUrl} alt={`${name || "User"} avatar`} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
            {showUserInfo && (
